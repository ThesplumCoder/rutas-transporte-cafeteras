...
tolls$UltimoFechaCambioPeaje = NULL
```

```{r}
hist(tolls$Valor, probability = TRUE)
...
```{r}
rutas$weight <- rutas$weight*0.09246022 #Conversión a galones
rutas$weight <- rutas$weight*9065 #Conversión a precio en pesos Colombianos
```

Finalmente, se suman los valores de combustible y peajes obtenidos para cada tramo.
```{r}
rutas$weight <- rutas$weight + valor_peajes
```
